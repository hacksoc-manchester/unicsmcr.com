...
const MainRouter = (database, passport) => {
  ...
  router.get(
    "/contact",
    authHelpers.attachReCAPTCHAKey,
    mainController.contact
  );
  // Contacting HackSoc
  router.post(
    "/contact",
    authHelpers.verifyReCAPTCHA,
    authHelpers.attachReCAPTCHAKey,
    mainController.contactHackSoc
  );
  // Message Page
  ...
  //Articles page
  const articlesRouter = require("./ArticlesRouter")(database, passport);
  ...
  router.get("/subscription/confirm", (req, res) => {
    ...
    res.redirect(
      301,
      `/signup/subscription/confirm?email=${email}&subscriptionId=${subscriptionId}&firstName=${firstName}&lastName=${lastName}`
    );
  });
  router.get("/subscription/remove", (req, res) =>
    res.redirect(
      301,
      `/signup/subscription/remove?email=${req.query.email}&subscriptionId=${req.query.subscriptionId}`
    )
  );
